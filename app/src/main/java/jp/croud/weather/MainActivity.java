...
public class MainActivity extends AppCompatActivity {
	...
	void load(final int city){
		new Thread(){
			...
			public void run() {
				WeatherData wd = null;
				try {
					URL url = new URL(String.format("http://weather.livedoor.com/forecast/webservice/json/v1?city=%d",city));
					ObjectMapper mapper = new ObjectMapper();
					wd = mapper.readValue(url,WeatherData.class);
					onLoad(wd);
				} catch (IOException e) {
					onLoad(null);
					e.printStackTrace();
	...
	private void setWeather(LinearLayout layout1, WeatherData.Forecasts forecast) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy年MM月dd日"); //java.text

		TextView date2 = (TextView) layout1.findViewById(R.id.textDate);
		ImageView image = (ImageView)layout1.findViewById(R.id.imageView);
	...
	void setImageURL(final ImageView imageView, final String url) {
		new Thread() {
			@Override
			public void run() {
